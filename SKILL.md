...
description: IP 信息管理工具，用于批量采集、存储、查询和导出 IP 地址的多维度情报信息。当用户需要查询 IP 信息、批量处理 IP 列表、导出 IP 数据到 Excel、对 IP 进行情报分析（Fofa/IPInfo/RDNS/Whois/爱站/站长之家）、管理 IP 数据库、溯源IP处理、IP自动分类、攻击IP分析、IP域名反查、域名DNS验证时使用此 skill。即使用户只是提到 "IP 查询"、"IP 信息"、"攻击 IP"、"威胁情报"、"IP 归属"、"IP 反查"、"批量查 IP"、"IP 导出"、"IP 反查域名"、"域名绑定"、"溯源"、"IP 分类"、"云主机识别"、"扫描器识别" 等关键词，也应触发此 skill。
---
...
│   └── ip_utils.py             # IP 工具
└── data/                       # 数据存储目录
    ├── {prefix}.json           # 主数据文件（由 IP_STORAGE_NAME 命名）
    ...
    │   ├── aizhan.log
    │   └── chinaz.log
    ├── {prefix}.trace_filtered_ips     # 溯源：需要深度查询的IP列表
    ├── {prefix}.unclassified_rdns      # 溯源：未识别RDNS记录
    ├── {prefix}.unclassified_no_info   # 溯源：信息不足IP
    ├── {prefix}.trace_report           # 溯源报告
    ├── {prefix}.domain_lookup_report   # 域名反查报告
    └── {prefix}.domain_lookup_matched  # 域名反查匹配结果
```
...
|------|---------|--------------|----------------|-------------|
| fofa | key | key 非空 + 调用 `/api/v1/info/my` | 省略，直接返回 JSON | 2 |
...
所有 IP 数据存储在 `{storage_dir}/{storage_name}.json`，每个 IP 下包含 `ip` 字段和若干渠道数据：

```json
{
  "1.2.3.4": {
    "ip": "1.2.3.4",
    "fofa": { "country_name": "...", "org": "...", "query_time": "..." },
...
```bash
python tools/progress_tool.py generate data/ip_data.json --channel fofa          # 从 JSON 生成 progress 文件
python tools/progress_tool.py generate data/ip_data.json --channel fofa -o custom.progress  # 指定输出路径
python tools/progress_tool.py remove data/ip_data.fofa.progress 1.2.3.4 5.6.7.8 # 删除指定 IP
python tools/progress_tool.py remove data/ip_data.fofa.progress --from-file ips.txt  # 从文件读取要删除的 IP
```
...
| 变量 | 必填 | 默认值 | 说明 |
|------|------|--------|------|
| `IP_STORAGE_DIR` | 否 | `data` | 数据存储目录（支持相对/绝对路径） |
...
| 阶段 | 说明 | 输出 |
|------|------|------|
| Phase 1 | 基础情报采集（IPInfo + RDNS，并行查询） | 数据写入 `{prefix}.json` |
| Phase 2 | 自动分类过滤 | `trace_classify` 渠道 + `{prefix}.trace_filtered_ips` + `{prefix}.unclassified_rdns` + `{prefix}.unclassified_no_info` |
| Phase 3 | 深度查询（爱站 + 站长 + Fofa，并行查询） | 数据写入 `{prefix}.json` |
...
- 未识别的 RDNS 记录输出到 `{storage_dir}/{prefix}.unclassified_rdns`
- 信息不足的 IP 输出到 `{storage_dir}/{prefix}.unclassified_no_info`
- `{prefix}` 由 `IP_STORAGE_NAME` 决定

**断点续跑**：每个阶段完成后写入标记文件（`{prefix}.trace_phase{N}_done`），使用 `--from-phase` 跳过已完成阶段。标记文件名由 `IP_STORAGE_NAME` 决定，不同批次互不干扰。
...
| 阶段 | 说明 | 输出 |
|------|------|------|
| Phase 1 | 域名收集（RDNS + 爱站 + 站长之家，并行查询） | `ip_domain_lookup` 渠道写入 `{prefix}.json` |
| Phase 2 | DNS 正向验证（批量并发验证域名是否仍解析到原 IP） | `verified_domains` 字段写入 `{prefix}.json` |
| Phase 3 | 汇总报告 | `{prefix}.domain_lookup_report` + `{prefix}.domain_lookup_matched` |
