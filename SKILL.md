...
8. **Word 报告自动生成** — 通过 `tools/docx_builder.py` 公共引擎，在流水线末尾自动生成公文排版格式的 Word 分析报告（需 python-docx）
9. **日志系统** — 通过 `scripts/logger_utils.py` 统一的 channel 和 batch 日志，支持控制台+文件双输出，自动轮转
...
│       ├── progress.py         # 进度管理 + 批量写入
│       └── reporter.py         # 报告生成（含 Word docx 报告）
├── tools/                      # 辅助工具
...
`ai_analysis.py` 用于从溯源IP数据中筛选待 AI 研判的 IP（按分类过滤：other/cloud_provider/residential），批量输出供人工或 AI 分析。研判结果通过 `writer.py add <IP> ai_analysis key=value ...` 写入，Word 报告中会自动展示 AI 研判结果章节。
...
可通过 `.env` 文件中的环境变量控制各阶段采集渠道的启用/禁用（默认全部启用）：
...
在 `.env` 文件中配置想禁用的渠道（例如阶段3不使用 Fofa Host）：
```bash
# 溯源IP流水线渠道配置
...
运行流水线时会显示每个渠道的启用/禁用状态，方便确认配置是否生效。
...
可通过 `.env` 文件中的环境变量控制各采集渠道的启用/禁用（默认全部启用）：
...
在 `.env` 文件中配置想禁用的渠道（例如不使用 ZoomEye 和 Fofa Search）：
```bash
IP_IP_DOMAIN_LOOKUP_ZOOMEYE_ENABLED=false
IP_IP_DOMAIN_LOOKUP_FOFA_SEARCH_ENABLED=false
```

运行流水线时会显示每个渠道的启用/禁用状态，方便确认配置是否生效。如果所有渠道都被禁用，流水线会报错并提示检查配置。
...
- Word 报告生成需要 `python-docx` 依赖（`pip install python-docx`），如未安装流水线会输出警告并跳过该阶段，不影响其他功能
