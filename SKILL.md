...
| 溯源 IP 自动化处理（采集→标签打标→分类→深度查询→DNS域名验证→溯源优先级分级→报告→AI研判） | references/trace-ip-pipeline.md | scenarios/trace_ip/ |
...
```bash
cd ~/skills/ip_info_manager
python3.11 tools/verify_ip_domain.py data/trace_ip/202605/202605.json
```

### 验证结果应用

- **优先级调整**：域名无法解析的 IP 可降低溯源优先级
- **威胁评估**：域名频繁变更说明攻击者活跃但基础设施不稳定
- **报告增强**：验证通过的映射会写入溯源报告的"IP-域名验证状态"章节

### 典型案例

2026 年 5 月数据验证发现：

- 154.201.64.225（高价值 IP）：38 个域名全部迁移到 47.238.119.42
- 222.186.34.244（P1 核心）：多个赌博平台域名已迁移
- 118 个域名中 72% 已变更或失效，说明攻击者频繁更换基础设施
