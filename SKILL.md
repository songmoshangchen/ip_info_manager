...
├── data/                       # 数据存储目录
│   ├── ip_data.json            # 主数据文件
│   ├── unclassified_rdns.json  # 未识别RDNS记录
│   └── trace_report.json       # 溯源报告
└── README.md                   # 详细文档
```
...
| Phase 2 | 自动分类过滤 | trace_classify 渠道 + trace_filtered_ips.txt + unclassified_rdns.json |
| Phase 3 | 深度查询（爱站 + 站长 + Fofa） | 数据写入 ip_data.json |
| Phase 4 | 汇总报告 | trace_report.json |
...
- 未识别的 RDNS 记录输出到 `data/unclassified_rdns.json`，可据此补充规则
...
**断点续跑**：每个阶段完成后写入标记文件（`.trace_phase1_done` 等），使用 `--from-phase` 跳过已完成阶段。
