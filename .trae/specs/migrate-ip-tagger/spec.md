...
* Affected code: `src/ip_info/batch/concurrent.py` (new), `src/ip_info/store/json_store.py` (扩展 progress\_tracker), `src/ip_info/store/in_memory.py` (扩展 progress\_tracker), `scripts/` (new)
...
| `batch/protocols.py`  | `ProgressTracker`         | 协议（is\_processed / mark\_processed）                                                                       | 进度跟踪协议（已完成）    |
...
| **脚本性质**            | 子类化 BaseBatchQuery（ABC）             | 实例化 BaseBatchQuery 或调用 run\_concurrent()           |
...
| **\_print\_result** | 每个脚本自定义打印格式                         | **移除**（通过日志和 BatchResult 替代）                       |
