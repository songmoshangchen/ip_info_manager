...
class BatchIPInfoQuery:
    def __init__(self, ip_file: str, channel_name: str = 'ipinfo_api'):
        ...
        try:
            with open(self.ip_file, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    ipinfo_data = self._query_ip(ip)
                    
                    if 'raw_error' in ipinfo_data:
                        print(f"❌ 错误: {ipinfo_data.get('error_message', 'Unknown error')}")
                    else:
                        country = ipinfo_data.get('country', 'N/A')
                        org = ipinfo_data.get('as_name', 'N/A')
                        print(f"✅ {country} - {org}")
                    
                    self.ip_writer.add_or_update_ip(ip, self.channel_name, ipinfo_data)
...
def main():
    if len(sys.argv) < 2:
        print("使用方法: python batch_ipinfo_api.py <IP文件路径>")
        print("示例: python batch_ipinfo_api.py ips.txt")
        sys.exit(1)
    ...
    batch_query = BatchIPInfoQuery(ip_file)
    batch_query.run()
