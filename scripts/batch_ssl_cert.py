...
class BatchSslCertQuery:
    def __init__(self, ip_file, channel_name='ssl_cert', no_validate=False):
    ...
    def run(self):
        ...
        self.logger.info("开始批量获取 SSL 证书域名")
        ...
        self.logger.info(f"连接端口: {self.settings.ssl_cert_port}")
        self.logger.info(f"连接超时: {self.settings.ssl_cert_timeout}s")
        ...
        try:
            for ip in self.pending_ips:
                ...
                if isinstance(data, dict) and data.get('error'):
                    self.logger.warning(f"[{current_count}/{total_count}] {ip} ❌ {data.get('error')}")
                    fail_count += 1
                else:
