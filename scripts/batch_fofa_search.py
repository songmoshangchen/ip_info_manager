...
class BatchFofaSearchQuery:
    def __init__(self, ip_file, channel_name='fofa_search', no_validate=False):
    ...
    def run(self):
        ...
        self.logger.info("开始批量查询 Fofa Search 信息")
        ...
        try:
            for ip in self.pending_ips:
                ...
                if isinstance(data, dict) and data.get('raw_error'):
                    ...
                    self.logger.warning(f"[{current_count}/{total_count}] {ip} ❌ API 错误: {data.get('errmsg', 'Unknown')}")
                    fail_count += 1
                else:
