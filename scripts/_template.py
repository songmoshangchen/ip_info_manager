...
class BatchXxxQuery:
    ...
    def _print_result(self, ip, data):
        ...
        示例:
          fofa:    logger.info(f"✅ {data.get('country_name')} - {data.get('org')}")
          aizhan:  logger.info(f"✅ {data.get('location')} - {data.get('domain_count')} 个域名")
          rdns:    logger.info(f"✅ {data.get('hostname')}")
        """
        pass
