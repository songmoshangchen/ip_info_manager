...
class TestBatchRunnerProtocolConformance:
    def test_isinstance_batch_runner(self):
        ...
        runner = BatchDnsVerify(ips=[], writer=writer, reader=reader)
...
class TestSkipIPsWithNoData:
    def test_ip_without_data_logs_warning(self, caplog):
        ...
        writer = InMemoryIPWriter()
        reader = InMemoryIPReader(data=writer._store)
        runner = BatchDnsVerify(ips=["10.0.0.1"], writer=writer, reader=reader)
        with caplog.at_level(logging.WARNING):
            result = runner.run()

        assert result.success_count == 0
        assert result.skip_count == 1
        assert reader.get_channel_data("10.0.0.1", CHANNEL_NAME) is None
    ...
    def test_mixed_ips_skip_only_no_data(self, mock_batch_verify):
        ...
        assert result.success_count == 1
        assert result.skip_count == 1
        assert reader.get_channel_data("1.2.3.4", CHANNEL_NAME) is not None
        assert reader.get_channel_data("1.2.3.4", CHANNEL_NAME)["matched"] == 1
        assert reader.get_channel_data("10.0.0.1", CHANNEL_NAME) is None
    ...
    def test_all_ips_no_data_returns_zero_success(self, caplog):
        ...
        writer = InMemoryIPWriter()
        reader = InMemoryIPReader(data=writer._store)
        runner = BatchDnsVerify(ips=["10.0.0.1", "10.0.0.2", "10.0.0.3"], writer=writer, reader=reader)
        with caplog.at_level(logging.WARNING):
            result = runner.run()
...
class TestDefaultExpiredBehavior:
    ...
    def test_expired_verify_with_force_days_none_not_reverified(self, mock_batch_verify):
        ...
        runner = BatchDnsVerify(ips=["1.2.3.4"], writer=writer, reader=reader, force_days=None)
        result = runner.run()
...
class TestForceDaysBehavior:
    ...
    def test_force_days_5_not_reverify_slightly_expired(self, mock_batch_verify):
        ...
        runner = BatchDnsVerify(ips=["1.2.3.4"], writer=writer, reader=reader, max_age_days=2, force_days=5)
        result = runner.run()
...
class TestParameterValidation:
    def test_max_age_days_zero_raises_value_error(self):
        ...
        with pytest.raises(ValueError, match="max_age_days"):
            BatchDnsVerify(ips=[], writer=writer, reader=reader, max_age_days=0)

    def test_max_age_days_negative_raises_value_error(self):
        writer = InMemoryIPWriter()
        reader = InMemoryIPReader(data=writer._store)
        with pytest.raises(ValueError, match="max_age_days"):
            BatchDnsVerify(ips=[], writer=writer, reader=reader, max_age_days=-1)

    def test_force_days_negative_raises_value_error(self):
        writer = InMemoryIPWriter()
        reader = InMemoryIPReader(data=writer._store)
        with pytest.raises(ValueError, match="force_days"):
            BatchDnsVerify(ips=[], writer=writer, reader=reader, force_days=-1)

    def test_max_age_days_one_is_valid(self):
        writer = InMemoryIPWriter()
        reader = InMemoryIPReader(data=writer._store)
        runner = BatchDnsVerify(ips=[], writer=writer, reader=reader, max_age_days=1)
        result = runner.run()
        assert result is not None
    ...
    def test_force_days_zero_is_valid(self):
        writer = InMemoryIPWriter()
        reader = InMemoryIPReader(data=writer._store)
        runner = BatchDnsVerify(ips=[], writer=writer, reader=reader, force_days=0)
        result = runner.run()
        assert result is not None
    ...
    def test_force_days_none_is_valid(self):
        writer = InMemoryIPWriter()
        reader = InMemoryIPReader(data=writer._store)
        runner = BatchDnsVerify(ips=[], writer=writer, reader=reader, force_days=None)
        result = runner.run()
        assert result is not None
