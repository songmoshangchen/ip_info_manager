...
class TestBatchRunnerProtocolConformance:
    ...
    def test_isinstance_batch_runner(self, tmp_path):
        ...
        classifier = BatchClassifier(ips=[], writer=writer, reader=reader, rules_dir=rules_dir)
...
class TestNormalBatchClassification:
    ...
    def test_classify_ip_with_rdns_data(self, tmp_path):
        ...
        classifier = BatchClassifier(ips=["1.2.3.4"], writer=writer, reader=reader, rules_dir=rules_dir)
    ...
    def test_classify_ip_with_ipinfo_data(self, tmp_path):
        ...
        classifier = BatchClassifier(ips=["5.6.7.8"], writer=writer, reader=reader, rules_dir=rules_dir)
        result = classifier.run()

        assert result.success_count == 1
        channel_data = reader.get_channel_data("5.6.7.8", CHANNEL_NAME)
        assert channel_data["category"] == "cloud_provider"
    ...
    def test_classify_multiple_ips(self, tmp_path):
        ...
        assert reader.get_channel_data("1.2.3.4", CHANNEL_NAME)["category"] == "cloud_provider"
        assert reader.get_channel_data("9.8.7.6", CHANNEL_NAME)["category"] == "cdn"
    ...
    def test_no_match_classified_as_other(self, tmp_path):
        ...
        classifier = BatchClassifier(ips=["10.0.0.1"], writer=writer, reader=reader, rules_dir=rules_dir)
        result = classifier.run()
