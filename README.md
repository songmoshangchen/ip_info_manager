...
- **溯源IP处理流水线**：自动采集、分类过滤、深度查询、溯源优先级分级、汇总报告
- **IP域名反查流水线**：域名收集、DNS正向验证、汇总报告
- **Word 报告自动生成**：流水线末尾自动生成公文排版格式的 Word 分析报告，溯源报告包含决策树优先级分级（P1-P4）、价值分级统计、动态溯源路径建议（需 python-docx）
...
| Phase 5 | 生成 Word 报告（含溯源优先级分级）    | `{project_name}.trace_report.docx`                                                                                                     |
...
| 三、溯源优先级 | 基于决策树模型的 P1-P4 分级（域名→端口→国内IP优先），含动态溯源路径建议 |
...
| P2 重点溯源 | 有反查域名（国外），或无域名但有端口信息（国内） | WHOIS查询域名注册信息；排查端口服务 |
| P3 辅助溯源 | 无域名但有端口信息（国外），或仅国内IP | 端口服务辅助分析；公开信息检索 |
