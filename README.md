...
- **多渠道情报采集**：通过 Fofa、IPInfo（API/免API）、RDNS PTR、Whois、爱站、站长之家、ZoomEye 等渠道批量查询 IP 信息
...
└── data/                   # 数据存储目录
    ├── {prefix}.json       # 主数据文件
    ├── logs/               # 日志目录
    ├── {prefix}.trace_filtered_ips
    ├── {prefix}.unclassified_rdns
    ├── {prefix}.unclassified_no_info
    ├── {prefix}.trace_report
    ├── {prefix}.domain_lookup_report
    └── {prefix}.domain_lookup_matched
```
...
所有 IP 数据存储在 `{storage_dir}/{storage_name}.json`，结构如下：
...
**标准函数**：`validate_channel_key()` → `request_channel()` → `fetch_channel()` → `main()`

**校验方式**：Key 非空 + 调用 `https://fofa.info/api/v1/info/my` 验证用户信息。

### channel/ipinfo_api.py — IPInfo 查询
...
| `batch_ipinfo_api.py` | ipinfo_api | `--no-api` | 批量 IPInfo 查询，支持免 API 模式 |
| `batch_rdns_ptr.py` | rdns_ptr | — | 批量 RDNS 查询（单线程） |
| `batch_rdns_ptr_concurrent.py` | rdns_ptr | `--workers N` | 批量 RDNS 查询（多线程，默认 5） |
...
- 未识别的 RDNS 记录输出到 `{prefix}.unclassified_rdns`
- 信息不足的 IP 输出到 `{prefix}.unclassified_no_info`
...
**断点续跑**：每个阶段完成后写入标记文件（`{prefix}.trace_phase{N}_done`），使用 `--from-phase` 跳过已完成阶段。支持 Ctrl+C 安全中断，自动保存进度。
...
| 阶段 | 说明 | 输出 |
|------|------|------|
| Phase 1 | 域名收集（RDNS + 爱站 + 站长之家 + ZoomEye，并行查询） | `ip_domain_lookup` 渠道写入 `{prefix}.json` |
...
### tools/merge_ip_files.py — IP 文件合并/去重/验证
...
### tools/config_tool.py — .env 配置管理工具
...
### tools/progress_tool.py — 进度文件管理工具

```bash
# 从 JSON 数据生成 progress 文件（将已有渠道数据的 IP 标记为已完成）
python tools/progress_tool.py generate data/ip_data.json --channel fofa
python tools/progress_tool.py generate data/ip_data.json --channel fofa -o custom.progress

# 从 progress 文件中删除指定 IP（用于重新查询特定 IP）
python tools/progress_tool.py remove data/ip_data.fofa.progress 1.2.3.4 5.6.7.8
python tools/progress_tool.py remove data/ip_data.fofa.progress --from-file ips.txt
```
...
| Settings 类 | 管理的配置项 |
|------------|------------|
| `BaseIPSettings` | `storage_dir`、`storage_name`（所有渠道共享） |
