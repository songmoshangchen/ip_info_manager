...
### channel/fofa_host.py — Fofa Host 聚合查询
...
### channel/fofa_search.py — Fofa 搜索查询
...
需要配置 `IP_FOFA_API_KEY`（与 fofa_host 共享）。支持 `query_suffix` 可选参数，场景可自定义附加查询条件（如 ` && is_domain=true`）。默认每次请求 1 页 20 条记录。
...
### channel/ssl_cert.py — SSL 证书域名提取
...
| 脚本 | 渠道 | 独有参数 | 说明 |
|------|------|---------|------|
| `batch_fofa_host.py` | fofa_host | — | 批量 Fofa Host 聚合查询 |
| `batch_fofa_search.py` | fofa_search | — | 批量 Fofa 搜索查询 |
...
| `batch_ssl_cert.py` | ssl_cert | — | 批量 SSL 证书域名提取 |
...
| Phase 3 | 深度查询（爱站 + 站长 + Fofa Host，并行查询） | 数据写入 `{prefix}.json` |
...
| 阶段 | 说明 | 输出 |
|------|------|------|
| Phase 1 | 域名收集（RDNS + 爱站 + 站长之家 + ZoomEye + Fofa Search + SSL 证书，并行查询） | `ip_domain_lookup` 渠道写入 `{prefix}.json` |
