...
| `IP_TRACE_IP_PHASE4_DNS_VERIFY_ENABLED`   | 否  | `true` | 溯源IP流水线阶段4：启用 DNS 域名正向验证   |
| `IP_TRACE_IP_DNS_VERIFY_TIMEOUT`          | 否  | `3.0`  | DNS 域名验证超时（秒）                   |
| `IP_TRACE_IP_DNS_VERIFY_CONCURRENCY`      | 否  | `10`   | DNS 域名验证并发线程数                   |
| `IP_TRACE_IP_PHASE5_PORT_SCAN_ENABLED`    | 否  | `false` | 溯源IP流水线阶段5：启用端口扫描（默认关闭） |
| `IP_TRACE_IP_PORT_SCAN_ENGINE`            | 否  | `nmap` | 端口扫描引擎（目前支持 nmap）             |
| `IP_TRACE_IP_PORT_SCAN_NMAP_PATH`         | 否  | `nmap` | nmap 可执行文件路径                   |
| `IP_TRACE_IP_PORT_SCAN_TIMEOUT`           | 否  | `30`   | 单 IP 端口扫描超时（秒）               |
| `IP_TRACE_IP_PORT_SCAN_PORT_LIST`         | 否  | `config/port_scan/top1000.txt` | 端口列表文件路径            |
| `IP_TRACE_IP_PORT_SCAN_QUERY_DELAY`       | 否  | `0.5`  | 端口扫描间隔（秒）                     |
...
python -m scenarios.trace_ip ips.txt --no-dns-verify        # 跳过 DNS 域名验证
python -m scenarios.trace_ip ips.txt --no-port-scan         # 跳过端口扫描
python -m scenarios.trace_ip ips.txt --from-phase 2         # 从阶段2开始
python -m scenarios.trace_ip ips.txt --only-phase 5         # 只执行端口扫描
python -m scenarios.trace_ip ips.txt --generate-report      # 只生成报告（等同于 --only-phase 7）
python -m scenarios.trace_ip ips.txt --collect-only         # 只执行基础采集（等同于 --only-phase 1）
python -m scenarios.trace_ip ips.txt --port-scan-only       # 只执行端口扫描（等同于 --only-phase 5）
...
| Phase 4 | DNS 域名正向验证                      | 验证结果写入 `domain_verify` 字段 |
| Phase 5 | 端口扫描（nmap，验证历史端口 + Top1000）     | 扫描结果写入 `port_scan` 渠道 |
| Phase 6 | 汇总报告                           | `{project_name}.trace_report`                                                                                                          |
| Phase 7 | 生成 Word + Excel 报告（含溯源优先级分级） | `{project_name}.trace_report.docx`、`{project_name}.trace_report.xlsx` |

**排除已溯源IP（Phase 7）：**

通过 `--exclude-ips` 参数可在报告生成时排除已溯源的 IP，方便聚焦剩余待溯源 IP。排除文件为纯文本，每行一个 IP。

```bash
python -m scenarios.trace_ip ips.txt --generate-report --exclude-ips traced_ips.txt
python -m scenarios.trace_ip ips.txt --exclude-ips traced_ips.txt  # 完整流水线中也可使用，仅 Phase 5 生效
```

排除后的报告所有统计基于剩余 IP 重新计算，报告概述中会明确显示排除信息（原始数/排除数/剩余数）。排除文件中不在数据中的 IP 会被忽略并在日志中提示。
...
| 端口扫描结果 | nmap 实时扫描结果汇总（开放端口数、端口列表），历史端口验证状态 |
...
- 统一字段：IP、国家、ASN/组织、分类、分类说明、建议溯源路径、域名数、反查域名列表（含DNS验证状态标记，换行分隔）、端口数、开放端口列表、实时扫描端口数、实时开放端口列表、标签
- 自动筛选 + 冻结首行 + 自动列宽
...
| Phase 4 | `IP_TRACE_IP_PHASE4_DNS_VERIFY_ENABLED`| `true` | 启用 DNS 域名正向验证   |
| Phase 4 | `IP_TRACE_IP_DNS_VERIFY_TIMEOUT`       | `3.0`  | DNS 验证超时（秒）     |
| Phase 4 | `IP_TRACE_IP_DNS_VERIFY_CONCURRENCY`   | `10`   | DNS 验证并发线程数     |
| Phase 5 | `IP_TRACE_IP_PHASE5_PORT_SCAN_ENABLED` | `false`| 启用端口扫描（默认关闭）  |
| Phase 5 | `IP_TRACE_IP_PORT_SCAN_ENGINE`         | `nmap` | 端口扫描引擎          |
| Phase 5 | `IP_TRACE_IP_PORT_SCAN_NMAP_PATH`      | `nmap` | nmap 可执行文件路径    |
| Phase 5 | `IP_TRACE_IP_PORT_SCAN_TIMEOUT`        | `30`   | 单 IP 扫描超时（秒）   |
| Phase 5 | `IP_TRACE_IP_PORT_SCAN_PORT_LIST`      | `config/port_scan/top1000.txt` | 端口列表文件 |
| Phase 5 | `IP_TRACE_IP_PORT_SCAN_QUERY_DELAY`    | `0.5`  | 端口扫描间隔（秒）     |

**使用示例**：

在 `.env` 文件中配置（例如禁用 Fofa Host 并启用端口扫描）：

```bash
# 溯源IP流水线渠道配置
IP_PHASE5_PORT_SCAN_ENABLED=true   # 启用端口扫描
IP_PORT_SCAN_NMAP_PATH=C:\Tools\nmap\nmap.exe  # Windows nmap 路径
IP_PORT_SCAN_TIMEOUT=120             # 扫描超时120秒
IP_PORT_SCAN_BATCH_SIZE=10           # 每批处理10个IP
```

#### Phase 5 端口扫描

端口扫描功能使用 **nmap** 对目标IP进行实时端口探测，验证历史端口的当前状态并发现新增开放端口。该功能**默认关闭**，需显式启用。

**依赖安装：**

1. 下载并安装 [nmap](https://nmap.org/download.html)（Windows 建议安装到非含空格路径，如 `C:\Tools\Nmap\`）
2. 或使用包管理器安装：
   ```bash
   # Windows (Chocolatey)
   choco install nmap
...
| 环境变量 | 默认值 | 说明 |
|---------|--------|------|
| `IP_PHASE5_PORT_SCAN_ENABLED` | `false` | 启用端口扫描（默认关闭） |
| `IP_PORT_SCAN_ENGINE` | `nmap` | 扫描引擎（目前仅支持 nmap） |
| `IP_PORT_SCAN_NMAP_PATH` | `nmap` | nmap 可执行文件路径（Windows需完整路径） |
| `IP_PORT_SCAN_TIMEOUT` | `90` | 单IP扫描超时秒数（建议60-180） |
| `IP_PORT_SCAN_PORT_LIST` | `config/port_scan/top1000.txt` | 端口列表文件路径 |
| `IP_PORT_SCAN_BATCH_SIZE` | `10` | 批处理大小（每批处理IP数，中断时最多损失一批） |

**批处理与断点续跑：**

端口扫描采用**批处理模式**，每处理完一批（默认10个IP）后自动保存进度。支持断点续跑：

- **中断恢复**：Ctrl+C 或异常中断后，重新运行相同命令会自动跳过已完成的IP
- **进度机制**：基于集合运算过滤，不依赖文件顺序，更可靠
- **损失控制**：中断最多损失一批（默认10个IP）的扫描进度

**扫描范围：**

1. **历史端口验证** — 扫描 Fofa/ZoomEye 返回的历史端口，验证当前是否仍开放
2. **Top 1000 端口** — 扫描最常见的 1000 个端口（可修改端口列表文件调整）
3. **合并扫描** — 两者合并去重后一次性扫描，提高效率

**输出数据：**

端口扫描结果写入 `port_scan` 渠道字段：

```json
{
  "port_scan": {
    "engine": "nmap",
    "scan_time": "2026-05-13T16:06:56",
    "host_alive": true,
    "open_ports": [
    ...
    "open_count": 1,
    "total_scanned": 496,
    "historical_ports_verified": [22],
    "historical_ports_closed": [80, 443]
  }
}
```
...
```bash
# 启用端口扫描（.env 配置）
IP_PHASE5_PORT_SCAN_ENABLED=true
IP_PORT_SCAN_NMAP_PATH=E:\01Tools\NMAP\nmap.exe
...
- 扫描结果可能受目标防火墙影响，部分端口显示为 `filtered` 而非 `open/closed`
- Windows 上 nmap 需要 TCP Connect（`-sT`），速度比 Linux 的 SYN 扫描慢
- 建议根据网络状况调整 `IP_PORT_SCAN_TIMEOUT` 和 `IP_PORT_SCAN_BATCH_SIZE`
```
...
| `TraceIPSettings`        | `phase1_ipinfo_enabled`、`phase1_rdns_ptr_enabled`、`phase3_aizhan_enabled`、`phase3_chinaz_enabled`、`phase3_fofa_host_enabled`、`phase4_dns_verify_enabled`、`dns_verify_timeout`、`dns_verify_concurrency`、`phase5_port_scan_enabled`、`port_scan_engine`、`port_scan_nmap_path`、`port_scan_timeout`、`port_scan_port_list`、`port_scan_query_delay` |
