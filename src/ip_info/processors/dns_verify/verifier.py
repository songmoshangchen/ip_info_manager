...
def resolve_domain(domain: str, timeout: float = 3.0) -> list[str] | None:
    try:
...
def batch_verify(
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for i, item in enumerate(candidates):
            future = executor.submit(verify_one, item["domain"], item["target_ip"], timeout)
        ...
        for future in as_completed(future_to_idx):
            ...
            try:
                ...
                item = candidates[idx]
                result = {"domain": item["domain"], "status": "error", "resolved_ips": []}
...
def build_verify_results(candidates: list[dict], verify_results: list[dict]) -> dict[str, list]:
