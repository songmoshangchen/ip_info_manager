...
| 阶段 | 说明 | 使用渠道 | 输出 |
|------|------|---------|------|
| 标签打标 | IP 威胁情报匹配（Phase 2 中自动执行） | ip_tagger（35 个情报源） | tags 字段写入 JSON |
...
| Phase 3 | 深度查询 | 爱站 + 站长之家 + Fofa Host（并行）+ DNS 域名正向验证 | 数据写入 JSON + domain_verify 字段 |
| Phase 4 | 汇总报告 | — | .trace_report |
| Phase 5 | Word + Excel 报告 | docx_builder + excel_exporter | .trace_report.docx + .trace_report.xlsx |

> 默认执行 Phase 1-5。Phase 6（汇总报告）和 Phase 7（Word + Excel 报告）需通过 `--summary-only` / `--generate-report` 单独触发。
...
Phase 5 同时生成 Excel 文件（`.trace_report.xlsx`），包含 4 个 sheet：
...
**统一字段：** IP、国家、ASN/组织、分类、分类说明、建议溯源路径、域名数、反查域名列表（换行分隔）、端口数、开放端口列表、标签
...
python tools/config_tool.py set IP_TRACE_IP_PHASE3_DNS_VERIFY_ENABLED false  # 禁用 DNS 域名验证
python tools/config_tool.py set IP_TRACE_IP_DNS_VERIFY_TIMEOUT 5.0           # DNS 超时5秒
python tools/config_tool.py set IP_TRACE_IP_DNS_VERIFY_CONCURRENCY 20        # 20线程并发
```
...
- **Phase 5 需要**：`python-docx`（Word 报告）、`openpyxl`（Excel 报告）
- 缺失时会输出清晰的错误信息和安装命令，并终止执行
- 如仅需执行 Phase 1-4，不安装这些依赖也可正常运行
...
Phase 5 报告生成时可通过 `--exclude-ips` 排除已溯源的 IP，使报告聚焦于剩余待溯源 IP。
...
# 完整流水线中也可使用，Phase 1-4 正常全量处理，仅 Phase 5 排除
...
- 排除仅在 Phase 5（Word + Excel 报告生成）生效，Phase 1-4 不受影响
